                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr v-for="comment in comments" :key="comment.id" class="hover:bg-gray-50 transition">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div>
                                            <div class="text-sm font-medium text-gray-900" v-text="comment.author"></div>
                                            <div class="text-sm text-gray-500" v-text="comment.email"></div>
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <a v-if="comment.url" :href="comment.url" target="_blank" class="text-xs text-blue-500 hover:underline mt-1 inline-block">
                                        <i class="fa-solid fa-link"></i> 访问网站
                                    </a>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
    ...
    createApp({
        setup() {
            const state = reactive({
                ...
                apiKey: '',
                config: {
                    baseUrl: localStorage.getItem('apiBaseUrl') || 'https://aapi-momo.motues.top' 
                },
                loginForm: { name: '', password: '' },
                comments: [],
                pagination: { page: 1, limit: 20, total: 1 }, // 默认total设为1防止闪烁
            ...
            const fetchComments = async (page = 1) => {
                ...
                try {
                    ...
                    // 提取分页对象
                    let pageData = result.pagination;

                    // 兜底处理
                    if (!pageData) {
                        pageData = { page: page, limit: 10, total: 1 }; // 默认值
                    }
